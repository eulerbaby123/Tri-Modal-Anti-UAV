...
# Tri-Modal Anti-UAV Dataset and ATMF-Net（多模态反无人机系统，可见光+红外+event）
...
*   **Multi-modal Alignment**: The different sensors have varying native resolutions: the RGB camera captures at 40x360 pixels, the thermal infrared camera at 640x512 pixels, and the event camera (DAVIS346) at 346x260 pixels. Given these differences in sensor specifications (resolution, field of view) and their physical arrangement, a feature-point-based registration technique is employed to align RGB and event frames to the coordinate system of the thermal modality (which has the highest native resolution among the provided figures, though it's important to double-check if 40x360 for RGB is a typo or specific cropping). An affine transformation matrix is applied to ensure that regions containing UAVs are spatially consistent across the three modalities. A "weak alignment" strategy is adopted, meaning it does not enforce strict pixel-level correspondence of UAVs within bounding boxes and also allows for weak alignment of background regions, aiming to encourage the development of robust fusion mechanisms insensitive to slight spatial inconsistencies.
