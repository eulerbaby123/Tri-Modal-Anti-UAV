...
def create_dataloader_rgb_ir_event(path1, path2, path3, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='', sampler=None):
    # 创建三模态数据加载器 - RGB、IR和Event
    with torch_distributed_zero_first(rank):
        dataset = LoadTriModalImagesAndLabels(path1, path2, path3, imgsz, batch_size,
                                      augment=augment,  # augment images
                                      hyp=hyp,  # augmentation hyperparameters
                                      rect=rect,  # rectangular training
                                      cache_images=cache,
...
class LoadTriModalImagesAndLabels(Dataset):  # for training/testing
    """
    载入三模态数据（RGB、IR和Event）
    """
    def __init__(self, path_rgb, path_ir, path_event, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix=''):
        self.img_size = img_size
        self.augment = augment
        self.hyp = hyp
        ...
        try:
            ...
            # -----------------------------  event   -----------------------------
            for p_event in path_event if isinstance(path_event, list) else [path_event]:
                p_event = Path(p_event)  # os-agnostic
                if p_event.is_dir():  # dir
                    f_event += glob.glob(str(p_event / '**' / '*.*'), recursive=True)
                elif p_event.is_file():  # file
                    with open(p_event, 'r') as t:
                        t = t.read().strip().splitlines()
                        parent = str(p_event.parent) + os.sep
                        f_event += [x.replace('./', parent) if x.startswith('./') else x for x in
                                  t]  # local to global path
                else:
                    raise Exception(f'{prefix}{p_event} does not exist')

            self.img_files_rgb = sorted([x.replace('/', os.sep) for x in f_rgb if x.split('.')[-1].lower() in img_formats])
            self.img_files_ir = sorted([x.replace('/', os.sep) for x in f_ir if x.split('.')[-1].lower() in img_formats])
            self.img_files_event = sorted([x.replace('/', os.sep) for x in f_event if x.split('.')[-1].lower() in img_formats])

            assert (self.img_files_rgb and self.img_files_ir and self.img_files_event), (
                f'{prefix}No RGB images found', 
                f'{prefix}No IR images found',
                f'{prefix}No Event images found'
            )
            
        except Exception as e:
            raise Exception(f'{prefix}Error loading data from {path_rgb,path_ir,path_event}: {e}\nSee {help_url}')

        # 检查RGB缓存
        self.label_files_rgb = img2label_paths(self.img_files_rgb)
        cache_rgb_path = (p_rgb if p_rgb.is_file() else Path(self.label_files_rgb[0]).parent).with_suffix('.cache')
        if cache_rgb_path.is_file():
            cache_rgb, exists_rgb = torch.load(cache_rgb_path), True
            if cache_rgb['hash'] != get_hash(self.label_files_rgb + self.img_files_rgb) or 'version' not in cache_rgb:
                cache_rgb, exists_rgb = self.cache_labels(self.img_files_rgb, self.label_files_rgb,
                                                          cache_rgb_path, prefix), False
        else:
            cache_rgb, exists_rgb = self.cache_labels(self.img_files_rgb, self.label_files_rgb,
                                                      cache_rgb_path, prefix), False

        # 检查IR缓存
        self.label_files_ir = img2label_paths(self.img_files_ir)
        cache_ir_path = (p_ir if p_ir.is_file() else Path(self.label_files_ir[0]).parent).with_suffix('.cache')
        if cache_ir_path.is_file():
            cache_ir, exists_ir = torch.load(cache_ir_path), True
            if cache_ir['hash'] != get_hash(self.label_files_ir + self.img_files_ir) or 'version' not in cache_ir:
                cache_ir, exists_ir = self.cache_labels(self.img_files_ir, self.label_files_ir,
                                                        cache_ir_path, prefix), False
        else:
            cache_ir, exists_ir = self.cache_labels(self.img_files_ir, self.label_files_ir,
                                                    cache_ir_path, prefix), False
                                                    
        # 检查Event缓存
        self.label_files_event = img2label_paths(self.img_files_event)
        cache_event_path = (p_event if p_event.is_file() else Path(self.label_files_event[0]).parent).with_suffix('.cache')
        if cache_event_path.is_file():
            cache_event, exists_event = torch.load(cache_event_path), True
            if cache_event['hash'] != get_hash(self.label_files_event + self.img_files_event) or 'version' not in cache_event:
                cache_event, exists_event = self.cache_labels(self.img_files_event, self.label_files_event,
                                                        cache_event_path, prefix), False
        else:
            cache_event, exists_event = self.cache_labels(self.img_files_event, self.label_files_event,
                                                    cache_event_path, prefix), False

        # 显示缓存信息
        nf_rgb, nm_rgb, ne_rgb, nc_rgb, n_rgb = cache_rgb.pop('results')
        nf_ir, nm_ir, ne_ir, nc_ir, n_ir = cache_ir.pop('results')
        nf_event, nm_event, ne_event, nc_event, n_event = cache_event.pop('results')
        
        if exists_rgb:
            d = f"Scanning RGB '{cache_rgb_path}' images and labels... {nf_rgb} found, {nm_rgb} missing, {ne_rgb} empty, {nc_rgb} corrupted"
            tqdm(None, desc=prefix + d, total=n_rgb, initial=n_rgb)
        if exists_ir:
            d = f"Scanning IR '{cache_ir_path}' images and labels... {nf_ir} found, {nm_ir} missing, {ne_ir} empty, {nc_ir} corrupted"
            tqdm(None, desc=prefix + d, total=n_ir, initial=n_ir)
        if exists_event:
            d = f"Scanning Event '{cache_event_path}' images and labels... {nf_event} found, {nm_event} missing, {ne_event} empty, {nc_event} corrupted"
            tqdm(None, desc=prefix + d, total=n_event, initial=n_event)
        ...
        # 读取RGB缓存
        cache_rgb.pop('hash')
        cache_rgb.pop('version')
        labels_rgb, shapes_rgb, self.segments_rgb = zip(*cache_rgb.values())
        self.labels_rgb = list(labels_rgb)
        self.shapes_rgb = np.array(shapes_rgb, dtype=np.float64)
        self.img_files_rgb = list(cache_rgb.keys())
        self.label_files_rgb = img2label_paths(cache_rgb.keys())
        if single_cls:
            for x in self.labels_rgb:
                x[:, 0] = 0

        n_rgb = len(shapes_rgb)
        bi_rgb = np.floor(np.arange(n_rgb) / batch_size).astype(np.int64)
        nb_rgb = bi_rgb[-1] + 1
        self.batch_rgb = bi_rgb
        self.n_rgb = n_rgb
        self.indices_rgb = range(n_rgb)

        # 读取IR缓存
        cache_ir.pop('hash')
        cache_ir.pop('version')
        labels_ir, shapes_ir, self.segments_ir = zip(*cache_ir.values())
        self.labels_ir = list(labels_ir)
        self.shapes_ir = np.array(shapes_ir, dtype=np.float64)
        self.img_files_ir = list(cache_ir.keys())
        ...
        n_ir = len(shapes_ir)
        bi_ir = np.floor(np.arange(n_ir) / batch_size).astype(np.int64)
        nb_ir = bi_ir[-1] + 1
        self.batch_ir = bi_ir
        self.n_ir = n_ir
        self.indices_ir = range(n_ir)
        ...
        # 读取Event缓存
        cache_event.pop('hash')
        cache_event.pop('version')
        labels_event, shapes_event, self.segments_event = zip(*cache_event.values())
        self.labels_event = list(labels_event)
        self.shapes_event = np.array(shapes_event, dtype=np.float64)
        self.img_files_event = list(cache_event.keys())
        self.label_files_event = img2label_paths(cache_event.keys())
        if single_cls:
            for x in self.labels_event:
                x[:, 0] = 0

        n_event = len(shapes_event)
        bi_event = np.floor(np.arange(n_event) / batch_size).astype(np.int64)
        nb_event = bi_event[-1] + 1
        self.batch_event = bi_event
        self.n_event = n_event
        self.indices_event = range(n_event)

        # Rectangular Training
        if self.rect:
            # RGB
            s_rgb = self.shapes_rgb
            ar_rgb = s_rgb[:, 1] / s_rgb[:, 0]
            irect_rgb = ar_rgb.argsort()
            self.img_files_rgb = [self.img_files_rgb[i] for i in irect_rgb]
            self.label_files_rgb = [self.label_files_rgb[i] for i in irect_rgb]
            self.labels_rgb = [self.labels_rgb[i] for i in irect_rgb]
            ...
            # Event
            s_event = self.shapes_event
            ar_event = s_event[:, 1] / s_event[:, 0]
            irect_event = ar_event.argsort()
            self.img_files_event = [self.img_files_event[i] for i in irect_event]
            self.label_files_event = [self.label_files_event[i] for i in irect_event]
            self.labels_event = [self.labels_event[i] for i in irect_event]
            self.shapes_event = s_event[irect_event]
            ar_event = ar_event[irect_event]

            # 设置训练图像形状
            shapes_event = [[1, 1]] * nb_event
            for i in range(nb_event):
                ari_event = ar_event[bi_event == i]
                mini, maxi = ari_event.min(), ari_event.max()
                if maxi < 1:
                    shapes_event[i] = [maxi, 1]
                elif mini > 1:
                    shapes_event[i] = [1, 1 / mini]

            self.batch_shapes_event = np.ceil(np.array(shapes_event) * img_size / stride + pad).astype(np.int64) * stride

        # 初始化图像数组
        self.imgs_rgb = [None] * n_rgb
        self.imgs_ir = [None] * n_ir
        self.imgs_event = [None] * n_event
    ...
    # 其它方法与LoadMultiModalImagesAndLabels相同，只是需要添加Event数据的处理
    def cache_labels(self, imgfiles, labelfiles, path=Path('./labels.cache'), prefix=''):
            # Cache dataset labels, check images and read shapes
            img_files = imgfiles
            label_files = labelfiles
    ...
    def __getitem__(self, index):
        ...
        index_event = self.indices_event[index]
        ...
        if mosaic:
            # 加载马赛克增强图像
            img_rgb, labels_rgb, img_ir, labels_ir, img_event, labels_event = load_mosaic_RGB_IR_Event(
                self, index_rgb, index_ir, index_event)
            shapes = None
        else:
            # 加载图像
            img_rgb, img_ir, img_event, (h0, w0), (h, w) = load_image_rgb_ir_event(self, index)

            # Letterbox
            shape = self.batch_shapes_rgb[self.batch_rgb[index]] if self.rect else self.img_size
            img_rgb, ratio, pad = letterbox(img_rgb, shape, auto=False, scaleup=self.augment)
            img_ir, ratio, pad = letterbox(img_ir, shape, auto=False, scaleup=self.augment)
            img_event, ratio, pad = letterbox(img_event, shape, auto=False, scaleup=self.augment)
        ...
        if self.augment:
            ...
            augment_hsv(img_rgb, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
            augment_hsv(img_ir, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
            augment_hsv(img_event, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])

        nL = len(labels_rgb)  # 标签数量
        if nL:
        ...
        # 转换图像格式
        img_rgb = img_rgb[:, :, ::-1].transpose(2, 0, 1)  # BGR转RGB, 转为3xHxW
        img_rgb = np.ascontiguousarray(img_rgb)
        img_ir = img_ir[:, :, ::-1].transpose(2, 0, 1)
        img_ir = np.ascontiguousarray(img_ir)
        img_event = img_event[:, :, ::-1].transpose(2, 0, 1)
        img_event = np.ascontiguousarray(img_event)

        # 合并三种模态的图像，9通道
        img_all = np.concatenate((img_rgb, img_ir, img_event), axis=0)
...
def load_image_rgb_ir_event(self, index):
    # --- 1. 从缓存中拿（可能为 None）---
    img_rgb   = self.imgs_rgb[index]
    img_ir    = self.imgs_ir[index]
    img_event = self.imgs_event[index]

    # --- 2. 如果缓存里全都为 None，就从硬盘读三模态图 ---
    if img_rgb is None and img_ir is None and img_event is None:
        path_rgb   = self.img_files_rgb[index]
        path_ir    = self.img_files_ir[index]
        path_event = self.img_files_event[index]

        img_rgb   = cv2.imread(path_rgb)   # BGR
        img_ir    = cv2.imread(path_ir)
        img_event = cv2.imread(path_event)

        assert img_rgb   is not None, f'Image RGB Not Found: {path_rgb}'
        assert img_ir    is not None, f'Image IR  Not Found: {path_ir}'
        assert img_event is not None, f'Image EVT Not Found: {path_event}'

    # 获取尺寸信息
    h0, w0 = img_rgb.shape[:2]  # 原始高宽
    ...
    if r != 1:  # if sizes are not equal
        ...
        img_ir = cv2.resize(img_ir, (int(w0 * r), int(h0 * r)),
                         interpolation=cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR)
        img_event = cv2.resize(img_event, (int(w0 * r), int(h0 * r)),
                         interpolation=cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR)
    
    # 返回值保持与调用处一致，只返回5个值
    return img_rgb, img_ir, img_event, (h0, w0), img_rgb.shape[:2]  # img_rgb, img_ir, img_event, hw_original, hw_resized
...
def load_mosaic_RGB_IR_Event(self, index1, index2, index3):
    # loads images in a 4-mosaic for RGB, IR and Event data
    ...
    # 确保索引一致
    assert index_rgb == index_ir == index_event, 'RGB、IR和Event的索引必须相同'

    indices = [index_rgb] + random.choices(self.indices_rgb, k=3)  # 3 additional image indices

    for i, index in enumerate(indices):
        # Load image
        img_rgb, img_ir, img_event, _, (h, w) = load_image_rgb_ir_event(self, index)

        # place img in img4
        if i == 0:  # top left
            img4_rgb = np.full((s * 2, s * 2, img_rgb.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            img4_ir = np.full((s * 2, s * 2, img_ir.shape[2]), 114, dtype=np.uint8)  
            img4_event = np.full((s * 2, s * 2, img_event.shape[2]), 114, dtype=np.uint8)
            x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
            x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
        elif i == 1:  # top right
            x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
        ...
        img4_event[y1a:y2a, x1a:x2a] = img_event[y1b:y2b, x1b:x2b]
        padw = x1a - x1b
        padh = y1a - y1b
        ...
        labels_rgb, segments_rgb = self.labels_rgb[index].copy(), self.segments_rgb[index].copy()
        labels_ir, segments_ir = self.labels_ir[index].copy(), self.segments_ir[index].copy()
        labels_event, segments_event = self.labels_event[index].copy(), self.segments_event[index].copy()
        
        if labels_rgb.size:
            # 归一化坐标转换为像素坐标
            labels_rgb[:, 1:] = xywhn2xyxy(labels_rgb[:, 1:], w, h, padw, padh)
            labels_ir[:, 1:] = xywhn2xyxy(labels_ir[:, 1:], w, h, padw, padh)
            labels_event[:, 1:] = xywhn2xyxy(labels_event[:, 1:], w, h, padw, padh)
            
            # 转换分割点
            segments_rgb = [xyn2xy(x, w, h, padw, padh) for x in segments_rgb]
            segments_ir = [xyn2xy(x, w, h, padw, padh) for x in segments_ir]
            segments_event = [xyn2xy(x, w, h, padw, padh) for x in segments_event]
            
        labels4_rgb.append(labels_rgb)
        segments4_rgb.extend(segments_rgb)
        labels4_ir.append(labels_ir)
        segments4_ir.extend(segments_ir)
        labels4_event.append(labels_event)
        segments4_event.extend(segments_event)

    # 合并标签
    labels4_rgb = np.concatenate(labels4_rgb, 0)
    labels4_ir = np.concatenate(labels4_ir, 0)
    labels4_event = np.concatenate(labels4_event, 0)
    ...
    for x in (labels4_ir[:, 1:], *segments4_ir):
        np.clip(x, 0, 2 * s, out=x)
    for x in (labels4_event[:, 1:], *segments4_event):
        np.clip(x, 0, 2 * s, out=x)

    # 数据增强：透视变换
    img4_rgb, img4_ir, img4_event, labels4_rgb, labels4_ir, labels4_event = random_perspective_rgb_ir_event(
        img4_rgb, img4_ir, img4_event, 
        labels4_rgb, labels4_ir, labels4_event,
        segments4_rgb, segments4_ir, segments4_event,
        degrees=self.hyp['degrees'],
        translate=self.hyp['translate'],
        scale=self.hyp['scale'],
        shear=self.hyp['shear'],
    ...
    # 确保标签一致性
    labels4_ir = labels4_rgb
    labels4_event = labels4_rgb

    return img4_rgb, labels4_rgb, img4_ir, labels4_ir, img4_event, labels4_event
...
def random_perspective_rgb_ir_event(img_rgb, img_ir, img_event, 
                                   targets_rgb=(), targets_ir=(), targets_event=(),
                                   segments_rgb=(), segments_ir=(), segments_event=(),
                                   degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
    # targets = [cls, xyxy]
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            img_rgb = cv2.warpPerspective(img_rgb, M, dsize=(width, height), borderValue=(114, 114, 114))
            img_ir = cv2.warpPerspective(img_ir, M, dsize=(width, height), borderValue=(114, 114, 114))
            img_event = cv2.warpPerspective(img_event, M, dsize=(width, height), borderValue=(114, 114, 114))
        else:  # affine
            # 仿射变换处理三种模态
            img_rgb = cv2.warpAffine(img_rgb, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            img_ir = cv2.warpAffine(img_ir, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            img_event = cv2.warpAffine(img_event, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
    ...
    if n:
        ...
        # 所有模态使用相同的转换结果（因为它们应该具有相同的标注）
        targets_rgb = targets
        targets_ir = targets.copy()
        targets_event = targets.copy()

    return img_rgb, img_ir, img_event, targets_rgb, targets_ir, targets_event
